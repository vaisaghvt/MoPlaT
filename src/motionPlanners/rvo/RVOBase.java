...
public abstract class RVOBase implements VelocityCalculator{
     ...
     *
     */
    protected double preferredSpeed = 1.3;
    /**
     * Maximum speed of agent - defined to be scalar multiple of
     * preferred velocity (1.3 chosen - with some
     * literature backing)
     */
    protected double maxSpeed = preferredSpeed * 2;
    /**
     * A maximum accelerator value - chosen to be 1. This is hard
     * to support with literature.
     */
//    protected double maxAccel = RVOAgent.MAXACCEL;
    protected double timeStep = 0.5;
     ...
     * @return
     */
    //TODO: vt:Apparently vector is not a good way to make a list. it is apparently slower. Should we change this?
    // and is this function used anyway?
    protected static Vector<Vector2d> lineCircleIntersection(Point2d p, double _x,
        ...
        if (Math.abs(_x) < 0.001) {
            ...
            if (delta < 0) {
                ...
                double x = p.x;
                vec.addElement(new Vector2d(x, y));
    ...
    protected double getMinTimeToCollision(RVOAgent me, Vector<ObjectTemp> objectsAround,
            Vector2d vCandidate, boolean collision) {
    ...
    protected Vector<ObjectTemp> getObjectsAround(RVOAgent me,
            Bag neighbors, Bag obses) {

        Vector<ObjectTemp> objectsAround = new Vector<ObjectTemp>();
        ...
        for (Object obj : neighbors) {
            ...
            if (collisionWithThis) {
                if (!collision) {
                    ...
                    //but no more.
                    objectsAround.removeAllElements();
                    collision = true;
                }
                objectsAround.addElement(new ObjectTemp(true,
        ...
        if (obses != null) {
            for (Object obj : obses) {
                ...
                if (collisionWithThis) {
                    ...
                    objectsAround.addElement(new ObjectTemp(false, tempObs.getPosition(), tempObs.getSize() * 0.707, null));
